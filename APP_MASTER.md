...
> ⭐ **27/9/2026 Đợt 408 — FIREBASE APP CHECK (theo dõi, CHƯA ép buộc) sau tấn công Tr0ngX**: `core/app-check.js` (bản chép myLesson web) nạp `defer` ở index/play/source; `core/firebase.js` dùng `getApps()` (không initializeApp hai lần). Không đổi luồng chơi/nộp. Chi tiết chặng Đợt 408.
...
> ⭐ **26/9/2026 Đợt 406 — ROCKET RACE FIGHT 3D: bỏ chữ LIFTOFF · Apply Options KHÔNG chạy lại cảnh phóng (cảnh đua + START 3D hiện sau ~0,4 s thay vì ~2,5 s dựng + ~10 s xem) · sau START chạm ĐÚP bất kỳ đâu ⇒ tua intro tới ~3 s trước câu hỏi** (chỉ `templates/rocket-race/`; ✅ ĐÃ PUSH `5a56068` + LIVE 6/6 mã băm; ⬜ chưa bấm tay TOMKO). Chi tiết chặng Đợt 406.
> ⭐ **26/9/2026 Đợt 405 — ROCKET RACE act VOICE: chạm thanh câu hỏi 3D (chữ 🔊) = nghe lại voice (Different: nửa trái/phải); act voice ⇒ nhạc nền TẮT cứng, dòng BACKGROUND trong menu 🔊 khoá mờ (không ghi đè lựa chọn đã nhớ), 2D cũng không bật nhạc** (chỉ `templates/rocket-race/`; ✅ ĐÃ PUSH `b73e96b` + LIVE; ⬜ chưa bấm tay TOMKO). Chi tiết chặng Đợt 405.
> ⭐ **26/9/2026 Đợt 404 — A SHOW SPEED FIGHT Mode 1: bàn thua (mất màu) hiện TỪ ĐÚNG của đội kia trong ô điền, chữ trắng rơi từng chữ, trong nhịp giữ trước câu mới** (template khai móc `reveal()` có sẵn của core → `m1Reveal`; không sửa core; ✅ ĐÃ PUSH `6cabc7c` + LIVE 2/2 mã băm; ⬜ chưa bấm tay TOMKO). Chi tiết chặng Đợt 404.
> ⭐ **26/9/2026 Đợt 403 — A SHOW SPEED rà 11 mục: chơi đơn BỎ HẲN bình (✓ ở góc là đủ) · Fight: ô đo lại khi đổi cỡ, ≤ 75 % bàn, bảng kết quả bỏ hàng số + thêm MISSED, bảng ANSWERS tối neon, tiếng hết giờ, điểm âm đếm xuống · bình vẽ nét theo cỡ trên màn (ws-lib) · CORE: `.aw-fight-review` z 31→44 (hàng nút không đè nữa, mọi Fight) · GAME: bỏ cổng rAF (khung ẩn thì ô không bao giờ trượt)** (✅ ĐÃ PUSH `904e1b8` + LIVE 5/5 mã băm; ⬜ chưa bấm tay TOMKO). Chi tiết chặng Đợt 403.
...
> Cập nhật lần cuối: **26/9/2026 (Đợt 399 — ghép MẪU 5c: tự giữ 60 khung + nhường card đồ hoạ trong myActivity; trước đó Đợt 398 — ROCKET RACE ▸ FIGHT 3D ghép MẪU 5b: cảnh phóng từ mặt đất + ván đua liền mạch; trước đó Đợt 397 — ROCKET RACE ▸ FIGHT 3D, thầy 26/9: (1) tia sáng đánh tàu thua = ĐÚNG loại vệt sáng bụi tốc độ bay suốt trận (dùng chính `dust.material`, cùng công thức độ dài vệt), bay vào từ MÉP MÀN (lùi dọc hướng bay tới khi ra khỏi khung hình) · (2) góc TOÀN CẢNH: đá vụn nhỏ trôi ngược dọc làn, tàu lượn NÉ bằng lò xo (mũi chĩa theo hướng né, thân nghiêng), góc đuổi không đổi (`cfg.dodge`, `applyDodge`) · (3) xác tàu XƠ XÁC + CHÁY RỤI + BỐC KHÓI: tấm vỏ mép rách random-walk, lỗ thủng, mép quăn, móp; cánh răng cưa (1–2 cánh gãy), chóp mũi/loa phụt/bơm toác hở, khung sườn gãy, dải vỏ xoắn, 24 mảnh vụn; mọi vật liệu pha muội + `vertexColors` loang lổ (mép rách đen kịt), hết bóng sơn; mọi mảnh đủ lớn bốc khói 6–15 s kéo thành vệt. Chi tiết `templates/rocket-race/GHI CHU ROCKET-RACE.md` mục 30. ✅ COMMIT + PUSH `fa41729`, live khớp mã băm, ⬜ thầy chưa xem thật)**. Trước đó **26/9/2026 (Đợt 396 — ROCKET RACE ▸ FIGHT 3D, 3 ý thầy (26/9): (1) bấm ◀ rời trang mà trận 3D vẫn chạy + vẫn kêu ⇒ core/fight.js có LƯỚI AN TOÀN (MutationObserver): khung trận rời trang ⇒ teardown() ⇒ huỷ cảnh WebGL + Web Audio, cùng khuôn `watchVanRoiTrang` Đợt 295 của engine · (2) vụ nổ hết KHỰNG (đo: khung nổ 95–435 ms → ~11 ms) — mọi thứ của vụ nổ dựng + BIÊN DỊCH SẴN lúc tạo cảnh, đèn chớp nổ luôn nằm trong cảnh; vụ nổ nhiều lớp lệch nhịp (chớp → 8 quả cầu lửa nhiễu → lửa cuộn → tia lửa + than hồng → 2 vòng sóng mảnh → khói bốc dần từ 12 nguồn, ám cam rồi nguội xám, mỗi hạt xoay một góc) thay cho một quả cầu + một cục khói · (3) XÁC TÀU dựng từ chính hình học con tàu (tấm vỏ cong mép rách, mặt trong cháy đen ửng đỏ rồi nguội, đai màu đội, mũi tách mảnh + chóp nguyên, 4 cánh — 1 cánh gãy, loa phụt, vòng buồng lái, kính vỡ, bồn nhiên liệu, khung sườn, ống đồng, bó dây, mảnh vụn), gắn ẩn trùng khít trong mô hình, lúc nổ tách ra đúng chỗ rồi văng theo pháp tuyến; mảnh lớn chậm/xoay chậm, vài mảnh còn cháy + kéo khói; không văng về máy quay. Chi tiết `templates/rocket-race/GHI CHU ROCKET-RACE.md` mục 29. ✅ COMMIT + PUSH `fa41729`, ⬜ thầy chưa xem thật)**. Trước đó **26/9/2026 (Đợt 395 — A SHOW SPEED (GAME `games/wordshake`): nút OPTIONS trên hàng nút (icon + tên level, chỉ bấm giữa các ván) chọn EASY/MEDIUM/HARD — `ws-lib rollBoard({level})` chọn bàn theo TỈ LỆ từ của dải mức (từ điển 55 % C-level nên đếm số lượng vô dụng), EASY giữ nguyên nhánh cũ; MISSED lọc theo level · hết giờ: hai số đếm CÙNG NHỊP từng nấc (≤ ~10 s), mỗi nhịp một tiếng tích cao + to dần, số đầu tiên vượt bên thấp ⇒ ô đội dẫn phóng to rồi đếm tiếp (tank `countTo`/`landCount`, sfx `countTick`/`swell`). 395b: số không nảy, chỉ ô đội dẫn to 1 lần; đổi level trượt viên sáng, không dựng lại bảng; bỏ dòng gợi ý. Chi tiết `GHI CHU DU AN.md` Đợt 395. ✅ PUSH `d678cb7`, live 3/3 mã băm khớp, ⬜ thầy chưa nghe/bấm tay TOMKO)**. Trước đó **26/9/2026 (Đợt 394 — ROCKET RACE ▸ FIGHT 3D, 3 ý thầy (26/9): (1) chữ ô đáp án 3D CỐ ĐỊNH cỡ, to hơn 15% (0.45 → 0.5175 × cao ô chuẩn); nhiều chữ ⇒ xuống dòng ở dấu cách + Ô CAO LÊN, KHÔNG co chữ, các dòng căn giữa ô (chỉ một TỪ dài hơn bề ngang ô mới buộc co); cột ô dài quá khung thì bám mép trên và mọc xuống phần trống dưới bàn (tới 90% cao cảnh), vẫn không đủ mới co đều cả cột · (2) nút ☰ MENU trong trận 3D trước không hiện gì (menu engine dựng trong bàn 0 đang ẩn, bị canvas che, hoạt ảnh aw-pop đứng ở khung 0) ⇒ template bê phần tử menu sang lớp phủ riêng `.aw-rr3d-menuhost` căn GIỮA màn, áo kính tối viền xanh (tiêu đề MENU, Resume nút vàng), đóng/mở vẫn do engine · (3) mở act / chọn template Rocket race ⇒ VÀO THẲNG FIGHT (cờ template mới `fightByDefault`, core/engine.js); rời Fight bằng MODE → Back to single thì act đó ở lại single (Start again/Apply không tự nhảy lại). Chi tiết `templates/rocket-race/GHI CHU ROCKET-RACE.md` mục 28. ✅ COMMIT + PUSH `4ae4e25`, live 5/5 mã băm khớp, ⬜ thầy chưa bấm tay TOMKO)**. Trước đó **26/9/2026 (Đợt 393 — ROCKET RACE ▸ FIGHT 3D chỉnh theo 13 ý thầy (26/9): TIẾNG hiệu ứng thay TOÀN BỘ bằng tiếng tổng hợp kiểu điện ảnh (`tools/rr3d-tao-am-thanh.py`, bỏ hẳn giọng đọc; đếm 3-2-1 = ting) phát bằng WEB AUDIO (lặp không khựng, tắt/bật trượt âm lượng, tiếng tăng tốc lắng dần ~6 s) · nút 🔊 trong trận 3D mở bảng EFFECTS / BACKGROUND (nhớ theo máy) · iPad bật ⇒ màn chính ẨN hẳn thanh chữ (kể cả khi iPad chưa báo có mặt), iPad chỉ còn TỪ ĐƠN · mọi kiểu kết trận (về đích, hết mạng, hết câu/giờ) đều chạy đủ cảnh: tàu thắng xuyên cổng, vệt sáng đánh tàu thua, cháy, nổ · màn kết quả RIÊNG nổi trên cảnh 3D (máy quay trôi quanh đám mảnh vỡ) qua hook core mới `sceneHandle.showResult()` · chữ mở màn nhỏ còn ~½ · turbo nổ dữ dội · nhãn hàng nút = % câu cả 2 bàn đã qua · trả lời sai: đuôi tàu xịt khói đen, lửa chỉ yếu đi không tắt · thanh câu hỏi dãn ngang theo độ dài câu (tối đa 176 cm) · đồng hồ trận dừng khi đã phân thắng thua (`ui.stopTimer` mới). Chi tiết `templates/rocket-race/GHI CHU ROCKET-RACE.md` mục 27. ⬜ thầy chưa bấm tay TOMKO)**. Trước đó **26/9/2026 (Đợt 392 — ⭐⭐ ROCKET RACE ▸ FIGHT 3D (thầy duyệt mẫu 2i ở kho myGame, "ok build" 26/9): trận Fight là MỘT cảnh WebGL (three.js r170 chép vào `templates/rocket-race/vendor/three/`, import ĐỘNG chỉ trong trận) phủ cả khung — camera đuổi theo 2 tàu, cột đáp án 3D sát hai mép, hàng nút kiểu game (bỏ ‹ ›, nhãn "câu / tổng"), mở màn ANDREW CLASSES → ROCKET RACE → START, kết trận tàu thắng bay khỏi màn → cổng co lại → vệt sáng đánh tàu thua → cháy → nổ vỡ vụn; bộ tiếng THẬT CC0 (Kenney + OpenGameArt, `sfx/`, qua core/sfx.js). CORE (thêm, tương thích ngược): `fightFrame.fullscene` + `tpl.fightScene({root, ctl, title, play})` + `destroy()` trong teardown (`core/fight.js`), `.is-fullscene` (`core/app.css`). Luật chơi/trọng tài/Lives/Points off/Different/iPad/Sudden death GIỮ NGUYÊN — code 2D cũ vẫn chạy trong một ổ ẨN, mọi sự kiện được PHẢN CHIẾU sang cảnh 3D (`v3()`); WebGL hỏng ⇒ tự lùi về 2D. Chi tiết `templates/rocket-race/GHI CHU ROCKET-RACE.md` mục 26. ✅ COMMIT + PUSH `f502b20`, live 9/9 mã băm khớp + bản live chạy trọn START → câu hỏi 3D, 0 lỗi. ⬜ thầy chưa bấm tay TOMKO)**. Trước đó **25/9/2026 (Đợt 391 — ROCKET RACE ▸ FIGHT Different: đội hết câu mà chưa ai về đích ⇒ dùng lại câu cũ (xào lại) tới khi có tàu về đích; Count down hết giờ ⇒ tàu gần hơn bay về, tàu kia nổ; core `fight.js` cờ bàn tuỳ chọn `recycleWhenOut`. Chi tiết `templates/rocket-race/GHI CHU ROCKET-RACE.md` mục 25. ✅ ĐÃ COMMIT + PUSH `9f8e5d4`, live khớp mã băm, ⬜ thầy chưa bấm tay)**. Trước đó **25/9/2026 (Đợt 390b — bình điểm A Show Speed đổi sang MẪU 4 thầy chọn (dải năng lượng neon + sao chổi, `ws-lib.js`), mức CỐ ĐỊNH ngang nhau mọi đội cho tới lúc cạn + đếm. ✅ ĐÃ COMMIT + PUSH, ⬜ thầy chưa bấm tay)**. Trước đó **25/9/2026 (Đợt 390 — A SHOW SPEED: BÌNH NƯỚC ĐIỂM (điểm ẩn trong bình, bay vào làm sóng sánh, hết giờ cạn nước + đếm số rồi mới hiện kết quả; GAME luôn có, template ô tích "Score tank"; core hook mới `tpl.fightReveal` trong `fight.js`) + mỗi lượt một bộ chữ khác (`rollBoard` nhớ 8 bàn gần nhất). ✅ ĐÃ COMMIT + PUSH cùng Đợt 389, ⬜ thầy chưa bấm tay)**. Trước đó **25/9/2026 (Đợt 389 — A SHOW SPEED TEMPLATE mở bằng màn START của GAME (2/3/5 phút = đếm ngược; Fight ở bảng giữa), ☰ ‹ › 🔊 ⛶ ra hàng ngoài kiểu nút game, ô tích NEXT mặc định tắt (lưu theo máy); CORE thêm 3 cờ tuỳ chọn `tpl.startScreen` · `tpl.toolsBelow` · `tpl.belowTools` — đọc `core/HUONG DAN CORE.md` mục Đợt 389 + `templates/wordshake/GHI CHU WORDSHAKE.md`. ✅ ĐÃ COMMIT + PUSH, ⬜ chưa bấm tay)**. Trước đó **25/9/2026 (Đợt 386 — ⭐⭐⭐ WORDSHAKE: GAME cố định `?g=wordshake` (fight 2 đội cùng 16 chữ, từ đội này lấy thì đội kia "Taken", bảng giữa hiện nghĩa, skin neon + âm thanh) + TEMPLATE #21 vào catalog (Mode 1 One word · Mode 2 Word list · Mode 3 Free words, Fight 3 mode); core `fightLayout:"shared-middle"` + `fightFrame.skin`; từ điển `templates/wordshake/ws-dict.txt` 18.345 từ có nghĩa Việt. Đọc `templates/wordshake/GHI CHU WORDSHAKE.md`)**. Trước đó **25/9/2026 rạng sáng (Đợt 384 — BÀI LÀM TỪNG CÂU cho dashboard myLesson v1.148.0: `ui.setReviewProvider` mới (engine `baiLamNay()`, quiz/type-the-answer/anagram/true-false/find-the-match/find-the-gap nối `() => buildReview()`) ⇒ lượt DỞ mang bài làm tới lúc dừng; practiceLog nhận `review` (gọn bằng `gonReview`: bỏ src, opts/img, kẹp 24 KB lúc keepalive) CHỈ cho lượt KHÔNG nộp, updateMask thêm "review" CHỈ ở lần ghi mang nó; results của NỘP DỞ nay có review. Luật `1436a49e` (practiceLog +review, results đọc `laThay()`). Đọc khối Đợt 384 đầu mục CHẾ ĐỘ HỌC SINH trong `core/HUONG DAN CORE.md`. Bàn thử dot383-play 47/47 + dot383-lo 9/9)**. Trước đó **25/9/2026 (Đợt 383 — ⭐⭐⭐ GỘP PRACTICE + SUBMIT: bài giao MỘT nút START (bia phi tiêu), lượt nào điểm ≥ 1 cũng NỘP (cả lượt DỞ khi Start again/tải lại/đóng tab — `doDang:true`, luật 3bf38eaa), màn nộp TỰ CHẠY (SUBMITTING + thanh xanh lá, lỗi sau 10 s), dải SUBMITTED, menu Start with mistakes · Show mistakes · Start again, Show answers → MY MISTAKES không bao giờ lộ đáp án (form tích sẵn), trong ván không lộ đáp án (`opt.anDapAn`: quiz/gameshow/rocket-race/find-the-gap/anagram), bỏ "Submit answers" khỏi menu ☰. Đọc khối Đợt 383 đầu mục CHẾ ĐỘ HỌC SINH trong `core/HUONG DAN CORE.md`. Bàn thử `scratch/dot383-play.html` 47/47 + `dot383-lo.html` 9/9)**. Trước đó **24/9/2026 tối (Đợt 382 — ROCKET RACE ▸ FIGHT thắng thua bằng VỀ ĐÍCH: đường đua cố định = nửa số câu (bỏ đường co giãn Đợt 355), chạm vạch ⇒ dừng ngay + tàu kia nổ; hết giờ/hết câu ⇒ tàu gần hơn bay về đích; hoà ⇒ SUDDEN DEATH câu ngẫu nhiên trong câu đã chơi; core `fight.js` thêm `ctl.finishRace`/`ctl.suddenDeath` + móc `roundsOver`/`resultScore` (tuỳ chọn). Chi tiết `templates/rocket-race/GHI CHU ROCKET-RACE.md` mục 24. ✅ COMMIT + PUSH, ⬜ thầy chưa bấm tay)**. Trước đó **24/9/2026 tối (Đợt 381 — SHOWDOWN tắt phím Andrew từ đầu: `ui.inShowdown()` mới trong `core/engine.js` (= `!!showdownPick`); Type the answer + Crossword khoá `extraKey` + dáng "used" + chốt trong `useAndrew()`; ✅ COMMIT + PUSH, ⬜ thầy chưa bấm tay)**. Trước đó **24/9/2026 (Đợt 380 — THỜI GIAN HOẠT ĐỘNG: practiceLog thêm `activeMs` = thời gian em THẬT SỰ làm (tab hiện + chạm/gõ trong 60 s, dạng đọc 180 s, hoặc âm không lặp đang phát; `play.js` `taoDoHoatDong`, không đụng core/engine), `timeMs` giữ nghĩa đồng hồ tường; treo > 10 phút ngừng ghi nhịp; luật đăng trước (myLesson `tools/dang-luat-active-ms.js`) — cho dashboard myLesson web v1.144.0)**. Trước đó **24/9/2026 (Đợt 379 — ĐIỂM LƯỢT BỎ DỞ: em bấm START AGAIN giữa ván ⇒ nhịp cuối `practiceLog` mang "điểm tới lúc dừng" (`diemBoDo()` trong `core/engine.js`, `leave({score})` trong play.js), không tính ván MISTAKES, không thêm lượt ghi — cho dashboard myLesson v1.141.0)**. Trước đó **23/9/2026 (Đợt 378 — ROCKET RACE ▸ FIGHT: Different khoá Time delay + Miss wait và hai bàn chạy độc lập KỂ CẢ KHI KHÔNG BẬT iPad — bỏ cổng `fightScreen` ở `soloBoards` + `syncSolo()` trong `core/fight.js`)**. Trước đó Đợt 377 — KHÔNG ZOOM: `touch-action: manipulation` cho mọi phần tử trong `app.css` + `core/no-zoom.js` chặn 2 ngón/Ctrl+lăn/Ctrl ± 0 + viewport `user-scalable=no`)**. Trước đó Đợt 376 — PWA (manifest chỉ ở index.html, icon iPhone + maskable).
> Trước đó: **23/9/2026 (Đợt 374 — ⭐⭐⭐ STATS: trang THỐNG KÊ ĐIỂM LỚP lưu dài hạn — icon biểu đồ cạnh icon Showdown ở trang chủ, `?st=1&c=<lớp>`; bảng hàng HS (myStudent theo mã, đọc `lessonWeb/lop`) × cột bài gom theo ngày (AWord = điểm lượt tốt nhất từ 22/9 · chấm tay = số lỗi, nhập chạm/vuốt); cây Năm→Tháng→Ngày; 📊 Đánh giá; ANALYSE = biểu đồ Showdown; Xuất Excel; chốt sổ vào `users/{uid}/items/st_<LỚP>_<YYYYMM>`. Chi tiết: `GHI CHU DU AN.md` Đợt 374)**.
> Trước đó: **23/9/2026 (Đợt 373 — GAME CỐ ĐỊNH ĐẦU TIÊN **WEREWOLF (ma sói)** ở gốc GAMES: màn quản trò iPad ngang cho trò chơi trực tiếp (ghi vai đêm 1, Witch cứu+độc, Seer, Bodyguard, Hunter có đường chỉ, sáng hiện người chết, đồng hồ thảo luận, vote vuốt thẻ + Confirm, nhạc WebAudio + mp3 IndexedDB, tự lưu ván). ⛔ **KHÔNG phải template** — thư mục mới `games/` (`games/games.js` `FIXED_GAMES`), `main.js` view `"game"` + link `?g=<id>`, `core/store.js` thêm kind `"werewolf"` vào `APP_DATA_KINDS`. Thêm game cố định = 1 mục trong `games/games.js`. Hồ sơ: `games/werewolf/GHI CHU WEREWOLF.md`. ✅ COMMIT + PUSH, ⬜ thầy chưa bấm tay iPad)**.
> Trước đó: **23/9/2026 (Đợt 372 — `tools/sinh-preload.py` BÁO LỆCH SAI ĐỜI ĐỜI, đã vá: `core.autocrlf=true` ⇒ git cất LF, đĩa CRLF, mà script sinh khối AW-PRELOAD + `core/tpl-files.js` bằng LF rồi **so chuỗi thô** ⇒ `"\r\n" != "\n"` ⇒ `--check` luôn LỆCH dù `--write` xong `git diff` RỖNG; và `--write` cũ ghép khối LF vào file CRLF làm `index.html` **hỗn hợp** 112 CRLF + 40 LF ⇒ git kêu "LF will be replaced by CRLF", phiên sau đi tìm lỗi ma. Vá: `norm()` chuẩn hoá **cả hai vế trước khi so**; `eol_of()` + `write_atomic(path, text, nl)` ghi theo kiểu ÁP ĐẢO của **chính file đó** (file mới ⇒ LF); ⛔ bắt buộc `"wb"` + `.encode()` vì `open(p,"w")` trên Windows dịch `\n` → `\r\n` LẦN NỮA ⇒ CRLF nhân đôi; thêm cảnh báo `⚠ TRỘN xuống dòng` (không tính là LỆCH) và `--write` nắn luôn file lai. Bench `scratch/dot372-eol.py` **30/30**, trong đó phép quyết định là **kho kiểu Linux toàn LF ⇒ vẫn ghi LF, nội dung khớp byte-đối-byte blob git**. Xoá được nợ 'sinh-preload LỆCH cả 3 mục' treo từ Đợt 299/300 — hoá ra script **không bao giờ báo KHỚP được**, không phải ai quên chạy. Chỉ `tools/sinh-preload.py`; `core/`/template/HTML không đổi một byte. 💡 ĐỀ XUẤT chưa làm: thêm `.gitattributes` — cố ý KHÔNG tự thêm vì đổi cách git xử lý toàn kho. ✅ COMMIT + PUSH)**.
> Trước đó: **22/9/2026 (Đợt 371 — ROCKET RACE ▸ MÀN iPad: chữ BÉ LẠI và **MỘT TỪ KHÔNG BAO GIỜ TÁCH HAI HÀNG** (thầy: *"chữ PENNY thì PENN hàng trên, Y hàng dưới"*, *"chữ trên ipad quá to"*). ⛔ **Gốc của "PENN / Y" là `overflow-wrap: break-word`** — ĐÚNG cái bẫy dự án đã ghi ở Đợt 222 (Find the match): *bẻ giữa từ xong thì ô HẾT TRÀN nên bộ thu chữ tưởng đã vừa và không co nữa*. Gỡ ⇒ chỉ xuống dòng ở DẤU CÁCH, và từ rộng hơn ô trở lại thành cú tràn THẬT mà phép đo thấy được. ⛔ **Bỏ `fitOnce`, tự đo bằng `Range.getClientRects()`**: `fitOnce` hỏi `scrollWidth` mà nó chỉ đếm phần tràn qua mép CUỐI — chữ ở đây CĂN GIỮA nên tràn đều CẢ HAI BÊN và scrollWidth báo thiếu (cùng lời nói dối đã ghi cho hộp flex căn giữa ở Find the match, cách chữa cũng chính là Range). Nay đo bề rộng DÒNG DÀI NHẤT + tổng cao từ line box. Chữ bé lại: base 7u→**5.5u** (hai nửa) / 9u→**7u** (gộp), **bỏ hẳn phóng to** (max 2.2→1 — cho nở là đúng ở Đợt 368b khi iPad còn hiện CÂU HỎI dài; từ Đợt 370 nó hiện một TỪ nên không còn gì để lấp đầy màn). **Hai nửa luôn cùng cỡ** = lấy MIN của hai bên (`layoutText`) — hai cỡ chữ cạnh nhau đọc ra thành "từ của đội này quan trọng hơn". Đo 1366×1024: PENNY/MARS ⇒ 1 dòng 75 px cùng cỡ · 15 ký tự ⇒ co 68 px vẫn 1 dòng · "Milky Way Galaxy Cluster" ⇒ 2 dòng CẮT Ở DẤU CÁCH · tiếng Việt 1 dòng · gộp ⇒ 96 px · từ 45 ký tự ⇒ fit .29 vẫn 1 dòng không tràn. ⚠️ Một phép đo của chính Claude tự sai (đọc `fontSize` SAU khi ca thử kế đã đổi `--fit` ⇒ báo 22 px cho ca lẽ ra 96 px) — đo lại từng ca ngay sau khi dựng. ✅ COMMIT + PUSH `95582f6` + LIVE. ⬜ thầy chưa bấm tay)**.
> Trước đó: **22/9/2026 (Đợt 370 — ROCKET RACE, 5 việc thầy giao một lượt: **(1)** công tắc QUESTION SCREEN rời bảng Options ra **nút riêng trên dải nút giữa Options và Mode** (đi ba đợt nhà: ô tích 368 → ô wide 368c → nút 370; bài học: thứ phải TÌM trong một bảng vẫn là thứ dễ bỏ sót, một nút SÁNG nói "đang bật" mà không ai phải mở gì). Khoá `rrTwoDevice` → **`fightScreen` (option của CORE)**; bấm → hỏi xác nhận → `ctl.applyOptions` (ghi act thật + lưu + dựng lại cả hai bàn về READY — restart là BẮT BUỘC vì cờ đọc MỘT LẦN lúc mount). **(2)** Different + màn iPad ⇒ **HAI BÀN CHẠY ĐỘC LẬP** (`core/fight.js`): mỗi bàn một `boardIdx` + **ô hẹn giờ RIÊNG** (⚠️ KHÔNG `later()` — đó là ô hẹn giờ vòng DUY NHẤT, hai bàn dùng chung sẽ huỷ câu tiếp của nhau, đúng bẫy `ctl.forfeit` đã né ở Đợt 354); `wordDone` rẽ nhánh SỚM trước cả bộ máy vòng chung; Time delay + Miss wait + Speed bonus bị khoá mờ. **BA CỔNG** (khuôn `turnsMode`): template khai `fightScreen` + act đang BẬT + `different`, cách ly pick-turn/In turns — thầy chốt phạm vi *"mọi game có và bật chế độ ipad"* nên cổng là TÍNH NĂNG, không phải tên template. ⚠️ Kết trận khi **CẢ HAI** hết câu, không phải bàn đầu: kết ở bàn đầu sẽ trao chiến thắng cho ai bấm nhanh nhất — mà bấm SAI không tốn thời gian, nên cách thắng nhanh nhất hoá ra là trả lời sai thật nhanh. 🐞 Lỗi tự bắt: khoá `cDelay` rồi để `syncDelay` mở lại ⇒ không chạy (syncDelay chỉ quản Speed bonus) ⇒ Time delay kẹt cứng khi về Same words — **hàm nào đóng cái gì phải tự mở được cái đó**. **(3)** `TAP_GUARD_MS` 400 ms chống double-tap (ô dựng lại ĐÚNG CHỖ ngón tay vừa bấm); ⚠️ phép đo đầu SAI vì đo `disabled` trong khi Solo chặn trong mã — đo lại bằng HÀNH VI: bấm ngay ⇒ câu không đổi, đối chứng ngược ⇒ đổi bình thường. **(4)** iPad hiện **TỪ** (đáp án đúng) thay câu gợi ý (`rrLinkText`→`rrLinkWord`); ⚠️ iPad nay hiện ĐÁP ÁN — thầy chọn có ý, là màn ĐỌC TO chứ không quay về phía lớp. **(5)** **nhạc nền vui** thay tiếng động cơ 55 Hz ỉ ảm (`rrSound.hum`→`music`): vòng I–vi–IV–V, square trên triangle, 132 BPM, **vẫn 100 % tổng hợp**; ⚠️ lên lịch theo ĐỒNG HỒ ÂM THANH (lookahead 40 ms) chứ không `setTimeout` — lệch timer vài chục ms là nghe ra nhịp khập khiễng; turbo làm nhạc NHANH LÊN. Hồi quy đo thật: Same words y hệt cũ; **QUIZ không đổi một chút nào** (không có nút mới, Different chỉ khoá Speed bonus). ✅ `23427d9`+`fc3d8ba`+`1961d39`+`5b8504d`. ⚠️ Claude KHÔNG NGHE ĐƯỢC nhạc — thầy phải tự nghe. ⬜ thầy chưa bấm tay)**.
> Trước đó: **22/9/2026 (Đợt 369 — FIND THE GAP ▸ MỐC GIÂY CHÍNH XÁC, chỉ sửa `tools/ftg-prepare.py` (KHÔNG đụng `core/`, không đụng template nào). Thầy báo "nghe không đủ, lệch chỗ" trên `LSA2-S3.T3.P1-2`. ⛔ **Gốc lỗi KHÔNG phải phép khớp mà là Parakeet NGHE SÓT cả đoạn băng**: băng 176,6 s nghe cả file ra 369 chữ nhưng bỏ trắng 157,4→162,5 s ("She's been ill. That man in the big hat is her husband, James.") và 170,5→172,6 s ("How people change!") — mất 18 chữ; `ffmpeg silencedetect` xác nhận hai chỗ đó **CÓ TIẾNG**, cắt riêng ra nghe lại bằng chính Parakeet ấy thì ra đủ ⇒ không phải model kém, mà nghe băng dài thì rụng đoạn. ⛔ **Đừng suy "ASR im lặng = băng im lặng" — phải hỏi ffmpeg.** Rồi `align()` cũ làm hỏng thêm: nó khớp **TỪNG DÒNG tham lam** (đường mốc giây còn chạy `whole=True` = mỗi dòng dò trên cả băng) nên dòng không có chỗ khớp **vẫn chộp đại một cửa sổ rồi ghi mốc**, chỉ gắn cờ `weak` chứ không từ chối — câu "She's been ill." bị gán vào chỗ phát "Oh. Isn't that" (khớp 0,00), câu kế đè ngược lên nó, câu cuối mất mốc, **11/50 câu chồng lấn** vì đệm cứng −0,15/+0,25 s không bám khe nghỉ thật. Ba tầng mới: ① `khe_im_lang`+`do_sot`+`nghe_va`+`va_asr` — dò vùng CÓ TIẾNG mà KHÔNG CÓ CHỮ ≥1,2 s (gọt hai đầu, lặp tới khi hai mép hết đổi, kẻo ôm đuôi im lặng ⇒ Parakeet đẻ chữ ma "Ugh"), ghép mọi vùng thành **MỘT wav vá** (nới ±0,8 s, chèn 1 s im lặng ngăn) rồi gọi Parakeet **đúng một lượt nữa**; bỏ chữ trùng **theo GIỜ** với chữ đã có (phần nới lấy lại "her."/"Oh" làm loãng phép căn); ghi đè cache `.pk.json` ⇒ lần sau và bước CLI sửa chữ hưởng ngay, và **tự dừng** (chạy lần 2 hết vùng sót); vùng nghe lại vẫn không ra chữ ghi sổ `<audio>.va.json`. Tắt bằng `--no-va`. ② `khop_toan_cuc`+`_dp_bang` — nối TOÀN BỘ câu hỏi thành một chuỗi rồi căn **một lần** bằng Needleman–Wunsch **dải hẹp quanh đường chéo** (`bang = max(250, |n−m|+150)`, `ptr` để `bytearray`) ⇒ không dòng nào chộp được chỗ của dòng khác, không dồn lệch dây chuyền; dòng 1–2 chữ ("One."/"Two.") chỉ cần 1 chữ khớp là đủ neo. ③ `suy_ra_va_cat` — cắt đầu/cuối tại **khe nghỉ thật** (≤0,7 s), hai câu chạm nhau thì cắt tại điểm giữa khe; câu không neo được thì suy từ hai hàng xóm chia theo số chữ + cờ **`suyra`** (nói thẳng thay vì bịa mốc). `@@KQ` thêm `suyra`/`sot_doan`/`sot_giay`/`va_chu`; myWord v2.8.0 hiện ba loại kết quả riêng và ghi `not heard - timing guessed from neighbours` vào cột H MATCH. **`align()` cũ GIỮ NGUYÊN cho đường tạo gói `.ftg.json`.** Đo lại trên chính bài của thầy: **khớp đúng 47/50 → 50/50 · sai chỗ hẳn 2 → 0 · mất mốc 1 → 0 · chồng lấn 11 → 0 · khớp trung bình 0,989**. Giá: `silencedetect` ~0,3 s; lượt Parakeet thứ hai **chỉ khi băng có đoạn sót** (đo thật 18,7 s) — băng sạch không tốn gì. Test mới `tools/thu-ftg-khop.py` (không cần ffmpeg/Parakeet/ổ D). ✅ **ĐÃ CHẠY TAY app myWord thật**: tab FIND THE GAP đang "mới có mốc giây 49/50" → bấm "Nghe băng lấy lại mốc giây" → "đã vá 17 chữ ở 2 đoạn băng máy nghe sót" → "✓ đủ mốc giây 50/50" → LƯU → sheet FILLGAP trong file thật 51 dòng đủ F/G/H, 0 chồng lấn, 15 sheet còn nguyên. **COMMIT + PUSH `90f9637`**)**.
> Trước đó: **22/9/2026 (Đợt 368 — ROCKET RACE · FIGHT TRÊN HAI MÁY (thầy: *"kết nối với 1 ipad… máy tomko sẽ không hiển thị câu hỏi nữa mà chỉ hiển thị race và khung chọn kết quả của 2 đội; ipad… chia đôi màn hình, mỗi bên hiển thị câu hỏi cho 1 đội"*): ô tích **Question screen** trong Options CỦA TRẬN + trang mới `source.html`/`source.js` ở gốc `web/` cho iPad (độc lập, KHÔNG nạp engine/template) + `templates/rocket-race/rr-link.js`. Một chiều: máy chơi GHI, iPad NGHE — iPad là MÀN HÌNH chứ không phải người chơi, nên **không có trạng thái chung phải tranh chấp**; hai tài liệu `rr_link`/`rr_view` trong `users/{uid}/items`, **mỗi cái đúng MỘT người ghi** ⇒ né được luật "nhiều bên ghi ⇒ transaction", và **KHÔNG phải đăng luật Firestore mới** (thầy chốt: iPad đăng nhập Google của thầy; tiếng đọc vẫn ở máy chơi; chỉ Rocket race trước). Giá ≈ **1 lượt ghi mỗi câu**. ⭐ Chống "cả lớp đứng hình": iPad đập nhịp 20 s, máy chơi **tự hiện lại** thanh câu hỏi khi nhịp quá 55 s; mặc định lúc mount là HIỆN, không bao giờ ẩn vì hy vọng. Đồng hồ chở MỐC (`clockMs`) rồi iPad tự đếm — 0 ghi thêm; ⛔ KHÔNG đưa thanh Time delay/Miss wait lên iPad (0,1–3 s mà mạng trễ 0,2–0,5 s = phép đo tự nói dối). Tận dụng được vì từ Đợt 355 câu hỏi ĐÃ tách khỏi ô đáp án (`.aw-rr-qbar` 2 nửa, tự gộp khi cùng chữ) ⇒ **KHÔNG đụng `core/fight.js` một dòng**; core chỉ sửa `core/store.js` (+3 chuỗi `APP_DATA_KINDS`, gồm vá `"showdown-session"` thiếu từ Đợt 269) — **ĐỀ XUẤT, chờ thầy duyệt**. Ba lỗi tự bắt ở bàn thử (chữ iPad co còn 41 % vì khối chữ không wrap; rồi chạm sàn 25 % vì `fitOnce` so chữ với `clientWidth` GỒM padding ⇒ **padding hộp chữ là chịu lực**; tràn 4 px với dấu tiếng Việt ⇒ slack 10). Đo thật: câu thường 72 px, câu dài 47 px, Việt 66 px không cắt, hai đội cùng câu ⇒ 92 px trải hết màn; đường đua +27 % (138,2→175,8 px), tâm lệch 0,2 px; **bật ô tích mà KHÔNG có iPad/Firebase ⇒ trận chạy y như thường, console sạch**. ⭐ **Đợt 368b — chỉnh riêng cho iPad Pro 12.9" M1 NẰM NGANG (1366×1024, thầy dùng cố định khổ này)**: chữ nay **NỞ** được chứ không chỉ co (`fitOnce` mặc định `max:1` ⇒ câu ngắn "Mars" giữ 96 px, lấp 15 % hộp; nâng `max:2.2` ⇒ **96→210 px**, câu dài/Việt KHÔNG đổi vì vốn đã bị chặn bởi chiều cao); `slack` nay **theo `u`** (`2×u`) vì dấu tiếng Việt vượt line box **tỉ lệ theo cỡ chữ** — 10 px sạch ở chữ 66 px mà CẮT ở chữ 88 px; ⛔ `contentBox:true` là bẫy (trừ padding khỏi bề rộng đem so trong khi khối chữ `width:100%` của chính content box ⇒ triệt tiêu, cả 5 câu thử tụt về sàn 24 px). Thêm **giữ màn hình không tự tắt** (`wakeLock`, iPad tự khoá ~2 phút mà màn này cả trận không ai chạm) + **`touch-action: manipulation`** (iOS bỏ qua `user-scalable=no` từ iOS 10). Chia TRÁI/PHẢI là cố ý (ánh xạ đúng vị trí 2 đội trên máy chơi). ⚠️ Bẫy thước đo: ảnh chụp bàn thử trông như trang chỉ chiếm góc trên-trái, nhưng `elementFromPoint(1360,1018)` = `.rrs-half` ⇒ **ảnh nói dối, số đo đúng**. 💡 Khuyên thầy "Add to Home Screen" để chạy toàn màn hình. ⬜⬜ **ĐƯỜNG MẠNG THẬT CHƯA CHẠY LẦN NÀO** — Claude không đăng nhập được tài khoản thầy; checklist 7 bước thầy bấm tay ở `templates/rocket-race/GHI CHU ROCKET-RACE.md` mục 20. ✅ **COMMIT + PUSH `0b5e4f1` + LIVE** — 7/7 mã băm SHA-256 khớp sau 220 s, `source.html` bản live mở đúng màn Sign in, 0 lỗi console; thầy duyệt gộp luôn sửa core `core/store.js`. ⭐ **Đợt 368c `ffad87d`** — ô tích "Question screen" thầy NHÌN KHÔNG RA (ô nằm ngay trên ảnh thầy gửi): gốc là `addCheck` làm nó rơi vào hàng Shuffle/Show answers — cả hàng là chuyện XÁO TRỘN nên mắt đọc **CỤM** rồi lướt qua ⇒ đổi thành `mkCell(wide)` + `mkSeg` **[Off | On iPad]**, dải trọn chiều ngang dưới LIVES. **Bài học: tính năng mới đặt lẫn vào nhóm CÓ SẴN thì thừa hưởng luôn ý nghĩa của nhóm đó.** ⭐ **Đợt 368d `cc4c27f`** — số câu **MỖI ĐỘI MỘT SỐ**, nằm DƯỚI khung đội đó (gói mang `qn0/qt0`+`qn1/qt1`; không thừa vì **In turns chia 81 thành 41/40**). Phần "độc lập" của LUẬT CHƠI thầy chốt **KHÔNG sửa** (*"tùy vào time delay như các act khác"*) — đã tra: `lockLoser()` (`fight.js:637`) **CHỈ true ở nấc Time delay 0,1s**, từ 0,2s/∞ thì đội chậm vốn đã KHÔNG bị khoá và giữ nguyên câu. 🐞 Bắt được `els.count` sót sau khi bỏ DOM ⇒ TypeError đúng lúc iPad về màn chờ. ⭐ **Đợt 368e+f `7751c0f`** — (1) tàu sát mép lúc xuất phát: `TRACK_START` 5→3 + **BỎ `.aw-rr-shared .aw-rr-lanes{left:7%}`** (đảo lại Đợt 360) ⇒ tàu 11,65u→**3u**, lửa→**0,28u**, khói→**−0,74u**; (2) **mũi tàu chạm vạch là thắng** — gốc: `--x` định vị **MÉP TRÁI** còn vạch đích đặt bằng số **CHÉP TAY** `TRACK_END+7` ⇒ vượt **3,78u**; sửa bằng **SUY RA**: bề rộng tàu thành **MỘT NGUỒN `--rw`** trên `.aw-rr-track` (thân + phi công + số mũi + vạch đều đọc), vạch = `calc((var(--track-end) + var(--rw)) * 1%)`, và `--lanes` phải **dời từ `.aw-rr-lanes` lên `.aw-rr-track`** vì vạch đích là **ANH EM** của khối làn chứ không phải con; đo lệch **0,00u** Fight / **0,01u** Solo dù tàu 11u vs 13u; bỏ `lanes left` còn lợi ích kín: **1 % của làn nay bằng đúng 1u**; (3) **nút icon-only cạnh Settings** (`main.js`, `icons.follow`) mở `source.html` **cùng tab** (Safari iPad nuốt popup, điều hướng thì không), cố ý **KHÔNG gắn với Rocket race** để act khác tái dùng — ⚠️ chưa xem được trên trang thật vì thanh nút chỉ dựng SAU khi đăng nhập; đã đo bằng cấu trúc `.aw-appbar-right` thật: **46×46 px bằng đúng nút Settings**)**.
> Trước đó: **22/9/2026 tối (Đợt 367 — MÃ HỌC SINH `ma` đi kèm điểm: `play.html?…&ma=<mã myStudent>` (myLesson web v1.134.0 gửi) → `queueAttempt/sendAttempt` ghi `ma` vào scores + results (chỉ khi có), `beatPlayLog` ghi `ma` vào practiceLog; luật 3 khối nhận `ma` tuỳ chọn ≤ 60 (ruleset `a648c2ea…`, lùi `7c630edf…`, script myLesson `app/tools/dang-luat-ma-hs.js` --kiem 15/15). Vì sao: myLesson khớp điểm theo TÊN, thầy đổi tên em bên myStudent là điểm cũ lạc/STAGE chặn cả lớp — nay web khớp theo mã trước, tên (kể cả tên cũ) sau. ⬜ thầy chưa bấm tay)**.
> Trước đó: **22/9/2026 (Đợt 366 — KHO LƯỢT LUYỆN `practiceLog` cho myLesson đo TỔNG THỜI GIAN LUYỆN TẬP + START AGAIN tách đôi (thầy chốt): màn kết thúc SUBMIT = SUBMIT HOMEWORK · Show answers · **Submit again** · **Practice again**; màn kết thúc PRACTICE = **Practice again** · Start with mistakes · **Start submitting** — nút nào vào THẲNG ván ở đúng chế độ (`startGame({hwPreset})`, tự START sau cổng chuẩn bị), không hỏi lại. Kho `practiceLog/{code}/entries/{id}` (luật ĐÃ ĐĂNG ruleset `7c630edf…`, lùi `fba064e2…`; script myLesson `app/tools/dang-luat-practice-log.js`): 1 tài liệu/lượt, REST PATCH ghi đè lúc START · MỖI 1 PHÚT · Game Complete (`done:true`+điểm) · rời ván/đóng tab (`keepalive`); ai cũng ghi, CHỈ THẦY đọc; lượt SUBMIT mang `attemptId`. HS đặc biệt (&db=1) không ghi. `core/assignments.js beatPlayLog/newPlayLogId`, `play.js session.playLog`, `core/engine.js enterGame/finish/cleanupAll/restartAs`. Đo thật: start → nhịp 60 s → xong 5/30 · 231 s · `done:true` → Start submitting vào thẳng SUBMIT → đóng tab ghi 19 s. ⛔ `createdAt` KHÔNG `| 0` (tràn 32-bit ra số âm — đã cắn). ⬜ thầy chưa bấm tay; dashboard myLesson đọc kho này ở web đợt 2)**.
> Trước đó: **20/9/2026 tối (Đợt 365 — FIND THE GAP, 3 ý thầy từ ảnh Fight FIND: (1) Fight + Random gaps hai bàn khoét KHÁC từ — sổ chung bị bàn 0 làm mới trong khi trọng tài mount bàn KIA trước (đo thật) ⇒ sổ mở một lần mỗi trận, khoá `text|start|end`, 16/16 câu trùng; (2) FIND ô đã bấm giấu chữ `•••` tới reveal; (3) `Min gaps` → thanh GAPS HAI NÚT min–max (`ftg-range.js` mới, vẽ theo thanh core, 20/20 phép cử chỉ), luật thầy chốt "thanh quyết định hoàn toàn": N hoặc bốc trong [min,max] mỗi câu mỗi ván, có thể ít hơn số thầy khoét (bench 219/219); ⚠️ act chưa có `maxGaps` ⇒ hi = 10. Chỉ `templates/find-the-gap/` + `core/tpl-files.js` sinh lại. ✅ COMMIT + PUSH `09f470e`, ⬜ thầy chưa bấm tay trang thật. Xem `templates/find-the-gap/GHI CHU FIND-THE-GAP.md` Đợt 365)**.
> Trước đó: **20/9/2026 khuya (Đợt 364 — QUIZ Time limit, 3 ý thầy sau khi xem trang thật: cụm [giây][thanh] cân ĐÚNG mép khối ô (ô số đo bằng bản nháp ẩn, cụm rộng = hàng ô trên, khớp 0,1 px); tích dồn dập 5 s cuối (`quizSound.tick`, 500 → 250 ms, cao dần); hết mạng KHÔNG còn nhạc 6 s thừa (`gameOver` im + `stopWarning()` ở finish). Chỉ `templates/quiz/`. ✅ COMMIT + PUSH `824aad0`, ⬜ thầy chưa bấm tay trang thật. Xem `templates/quiz/GHI CHU QUIZ.md` Đợt 364)**.
> Trước đó: **20/9/2026 tối (Đợt 363 — QUIZ: thanh TIME LIMIT 1s..20s + nấc ∞ trong Options, giới hạn giây MỖI CÂU ở MỌI mode (đơn · Fight · Showdown); hàng [giây][thanh] giữa câu hỏi và ô; hết giờ = sai qua đúng `roundTimeUp()` Đợt 174, Fight báo trọng tài `wordDone({correct:false})`; đóng băng khi trượt câu/Menu/clip đọc/bàn khoá; chỉ `templates/quiz/`, không đụng core. ✅ COMMIT + PUSH `a37686a`, ⬜ thầy chưa bấm tay trang thật. Xem `templates/quiz/GHI CHU QUIZ.md` Đợt 363)**.
...
> dẫn đầu CHẠM ĐÍCH đúng lúc kết trận (đo: 10 câu xen kẽ → 86/86, "IT'S A DRAW 5—5"). Quiz Fight hồi quy không đổi. Xem
> `templates/rocket-race/GHI CHU ROCKET-RACE.md` mục 12)**.
> Trước đó: **20/9/2026 tối (Đợt 354 — ROCKET RACE · FIGHT, ngay sau Đợt 353, 3 ý thầy: (1) BỎ ĐIỂM trên dải
> (`fightFrame.noScore`, điểm vẫn tính ngầm), sai + Points off ⇒ TÀU LÙI N nấc kèm "−N" bay từ tàu; (2) bỏ thẻ TEAM 1/2,
> in SỐ 1/2 lên mũi tàu; (3) LIVES vào Options trận: mỗi mạng mất tàu NÁT DẦN (3 mức: nứt · xỉn · khói, chia theo tỉ lệ mạng),
...
> -50%)` của phi công; lửa là anh em của thân nên không đi theo — chữa bằng khung bọc `.aw-rr-craft`, ăn cả Solo/Teams),
> ô đáp án 1 hàng 4, ☰/‹ ›/🔊 của bàn 0 DỜI lên dải nút chung cạnh Options/Mode (`fightFrame.boardTools:"shared"`), và
> **lỗi ngầm từ Đợt 351**: sân Rocket race phủ kín khung nên thanh Time delay của trọng tài bị che — nay `ui.hostFightWaitBar()`
> cho template nhận nuôi thanh ấy vào đáy panel. Core: `fight.js` (+`tpl.fightFrame` → `--aw-fsh/--aw-fbh`, dời node bàn 0),
...
> (cái xanh lá, cái xanh dương) — đôi khi phát âm 2 bên lệch nhau một chút, đặc biệt là khi có 1
> bên bấm. Cần đảm bảo chúng luôn chỉ là 1 ở chế độ fight."*
>
> ⛔ **THIẾU 4/4 MẢNH CỦA KHUÔN, CHỈ CÒN ĐÚNG CÁI RÀO.** Hợp đồng giọng đọc trong Fight: chỉ bàn 0
> sở hữu `<audio>` thật (`ctl.speaks`), bàn 1 chỉ nhại lại ánh sáng. Khuôn đủ (Đợt 259) có bốn
> mảnh — (A) `createVoicePlayer({onGlow})`→`reportVoiceState` · (B) `handleListenTap`→
> `requestVoiceToggle` · (C) `toggleVoiceRemote`+`syncVoice` trong `ctl.attach` · (D) nửa PULL
> `voiceState()`. `quiz` và `true-false` có **mỗi cái rào `speaks` ở autoPlay** và **không mảnh
> nào** trong bốn mảnh đó ⇒ (1) nút loa bàn kia không bao giờ sáng; (2) cú chạm ở nút loa
> **không có rào nào** nên bàn không-sở-hữu-tiếng tự phát bản thứ hai chồng lên — đúng lúc "có 1
> bên bấm". Đây là **nợ đã ghi từ Đợt 266** ("⬜ Quiz + True/false vẫn còn thiếu GƯƠNG… chờ thầy
> gật"); nay thầy gặp thật thì làm nốt, và làm **đủ bốn mảnh**.
>
> **Vá:** hai file `templates/quiz/quiz.js` · `templates/true-false/true-false.js`, cùng một bản
> vá chép đúng khuôn Crossword (+50/−3 mỗi file), thêm `handleListenTap()` và `let curVoiceBtn`
> (giữ bằng BIẾN, không `querySelector`: `.aw-tf-prompt` bị dựng lại và trượt ra/vào).
>
> ⚠️⚠️ **BÀN THỬ NÓI DỐI BỐN LẦN trước khi ra được số tin được** — đọc mục Đợt 302 trong
> `GHI CHU DU AN.md`, cả bốn sẽ cắn lại: (1) tìm nút loa bằng `.aw-fight-half` trong khi nó nằm
> ở `.aw-fight-board`; (2) đếm mọi `<audio>` nên đếm cả tiếng động của `core/sfx.js`; (3) ⛔⛔ ép
> **`muted = true`** để qua cửa autoplay — Chrome coi media đang tắt tiếng ở **tab nền** là
> "video-only background media" và **tự tạm dừng**, `play()` trả `AbortError`, hào quang tắt oan
> ⇒ phải dùng **`volume = 0`** cộng **một cú bấm THẬT** của người; (4) bắn cả `pointerdown` lẫn
> `click` trong khi `core/press.js` ăn cả hai ⇒ mỗi "cú chạm" thành hai lượt toggle.
> ⭐ Và phải có **mỏ neo DƯƠNG** (khẳng định nút bàn trái ĐANG sáng) — không thì phép so "hai nút
> cùng trạng thái" ĐẠT một cách RỖNG (`false === false`).
>
> Bàn thử `scratch/dot302-fight-voice.html` + `scratch/dot302-sweep.html`. A/B thật (stash bản vá
> rồi chạy lại): **4/27 TRƯỢT trước → 27/27 ĐẠT sau**, 4 phép trượt rơi đúng 2 triệu chứng × 2
> game; chế độ **một bàn** ĐẠT sạch ở cả hai lượt ⇒ nếp ngoài trận không xê dịch.
> `node --input-type=module --check` sạch cả 2 file.
> Quét 7 game: `quiz` ✅ (sau vá) · `find_the_match` ✅ (vốn đúng) · 5 game còn lại **KHÔNG ĐO
> ĐƯỢC** (nút loa không có mặt ngay lúc mount) — khai đúng thế, **không khai ĐẠT**.
> ⬜ **CHỜ THẦY BẤM TAY** một trận Fight act VOICE ở Quiz và True/false; và ⬜ **cho biết hôm nay
> thầy chơi game nào** — nếu là Anagram/Type the answer/Open the box/Crossword thì phải đo tiếp,
> 4 game đó đủ 4 mảnh trên giấy nhưng chưa đo được hành vi thật.
>
> ---
>
...
> kiểu tên"* — Đợt 290 chỉ né được tên 2 tiếng; tên 3+ tiếng vẫn có thể trùng giữa HAI ĐỘI KHÁC NHAU.
> Hỏi thầy 3 phương án, thầy tự chỉ cách thứ tư đúng hơn cả: *"ngay từ khi bắt đầu chọn các thành viên
> để chia bảng... quyết định rút gọn tên đó ngay từ đầu... rồi mới chia ra các cột sẽ không bị trùng
> nữa"* — quyết định nhãn NGAY LÚC BẤM READY (nơi còn thấy CẢ TRẬN, mọi đội gộp lại), mang nhãn đó
> theo `pick` sang từng cột, thay vì để mỗi cột tự tính sau khi đã tách rời. `assignFullLabels()` mới
> (`core/showdown.js`) tính nhãn không trùng cho cả trận một lần (mở dần từng tiếng, đánh số nếu trùng
> tên thật); `fullLabelMap()` (`core/showdown-setup.js`) gắn `pick.members[].shortLabel` ở
> `stampPickFromTable`/`applyReady`/`applySolo`; `sdRefreshFromTable()` (`core/engine.js`) cũng tính
> lại khi bảng đội đổi giữa chừng; màn READY ưu tiên đọc nhãn có sẵn, lùi về `shortenTeamNames()`
> (Đợt 290) cho pick cũ. `node --check` sạch cả 3 file; test độc lập với đúng kịch bản ảnh chụp thầy
> gửi + ca 3 tiếng trùng giữa 2 đội — hết trùng cả hai. Code `d3c6c8a` ĐÃ PUSH + LIVE kiểm chứng
> SHA-256 cả 3 file. Chi tiết `GHI CHU DU AN.md` Đợt 291. ⬜ chờ thầy bấm tay: 2 em tên 3 tiếng trùng
> chữ cái đầu ở hai đội khác nhau.
>
> ---
>
> **Đợt 290** (03/9/2026, thầy) — ⭐⭐⭐ **NỐI TIẾP ĐỢT 289: MÀN READY VẪN TRÙNG "T.LÂM" GIỮA HAI CỘT.**
> Tên 2 tiếng (VD "Tuệ Lâm") không còn viết tắt xuống 1 chữ cái theo mặc định — luôn hiện đủ ở mọi cột,
> độc lập, không cần phối hợp. ⬜ còn hở tên 3+ tiếng trùng giữa 2 đội — vá dứt điểm ở Đợt 291 ngay trên.
> Code `0d4bb09` ĐÃ PUSH + LIVE. Chi tiết `GHI CHU DU AN.md` Đợt 290.
>
> ---
>
...
> **⭐⭐⭐ Đợt 374 (23/9/2026) — STATS · ✅ ok build → COMMIT + PUSH · ⬜ CHƯA BẤM TAY.** Trang thống kê điểm lớp: `core/stats-home.js` +
> `core/stats.css` (nạp động), `main.js` view `"stats"` + icon + `?st=`, `store.js` kind `stats-month`, `showdown-setup.js` `renderLegend` cờ `dayOnly`.
> Bàn thử `scratch/dot374-stats.html` (Firestore giả `scratch/fake-firebase374.js`). ⛔ Danh sách lớp CHỈ từ `lessonWeb/lop`, không bao giờ `mystudentRosterStudents`.
> Thầy bấm tay theo ⬜ VIỆC ĐANG CHỜ của `GHI CHU DU AN.md` Đợt 374.
...
>
> **✅ 18/9/2026 khuya — THẦY NGHIỆM THU đường Import `.xlsm` → act FIND THE GAP (Đợt 343/344) trên trang thật với bài LSB1-S1.T1.P1 ("ok ngon rồi").** ⬜ Còn: chơi 3 mode + Fight/Showdown với câu hỏi gộp trên TOMKO.
>
...
> `<mã> / FIND THE GAP`, hàng riêng trong hộp Import; `parseBracketLine` = bản JS của tool. Đo: P1 → 3 act, 50/50 câu hỏi
> chơi được, 93 chỗ. Gói `.ftg.json` không còn cần với thầy (`ftg-prepare.py` thêm `--align-json`). ⬜ Chưa bấm tay hộp Import thật.
>
...
> | | |
> |---|---|
> | Commit mới nhất | **Đợt 302** — `templates/quiz/quiz.js` · `templates/true-false/true-false.js` (+50/−3 mỗi file). |
> | Thầy báo gì | *"Ở chế độ Fight và chơi với voice: 2 nút loa của 2 bên không đồng bộ với nhau (cái xanh lá, cái xanh dương) — đôi khi phát âm 2 bên lệch nhau một chút, đặc biệt là khi có 1 bên bấm. Cần đảm bảo chúng luôn chỉ là 1 ở chế độ fight."* |
> | Gốc rễ | **Thiếu 4/4 mảnh của khuôn, chỉ còn đúng cái rào.** Hợp đồng giọng đọc Fight: chỉ bàn 0 sở hữu `<audio>` thật (`ctl.speaks`), bàn 1 nhại lại ánh sáng. Khuôn đủ (Đợt 259, mẫu ở `templates/crossword/crossword.js`) gồm **A** `createVoicePlayer({onGlow})`→`reportVoiceState` · **B** `handleListenTap`→`requestVoiceToggle` · **C** `toggleVoiceRemote`+`syncVoice` trong `ctl.attach` · **D** nửa PULL `voiceState()`. `quiz` và `true-false` có **mỗi cái rào `speaks` ở autoPlay** và **không mảnh nào** trong bốn mảnh ⇒ (1) nút loa bàn kia không bao giờ sáng; (2) cú chạm ở nút loa **không có rào nào** nên bàn không-sở-hữu-tiếng tự phát bản thứ hai chồng lên bản đang chạy. |
> | Nợ có sẵn | Ghi chú Đợt 266 (26/8/2026) đã ghi *"⬜ Quiz + True/false vẫn còn thiếu GƯƠNG (đo live: nút loa sáng 7/28 bên trái, **0/28** bên phải) — nợ có sẵn, chờ thầy gật."* Đợt này làm nốt, và làm **đủ bốn mảnh** chứ không chỉ cái gương. |
> | Đã sửa | Hai file, **cùng một bản vá chép đúng khuôn Crossword**: `onGlow`→`reportVoiceState` (có rào `speaks()` để không dội ngược) · `handleListenTap()` mới · `toggleVoiceRemote`(có rào `speaks()`)+`syncVoice` trong `ctl.attach` · nửa PULL ở nhánh `else if (fightCtl)` của autoPlay · `let curVoiceBtn` giữ nút loa của câu đang hiện. ⚠️ Giữ bằng **BIẾN**, không `querySelector`: `.aw-tf-prompt` bị dựng lại và trượt ra/vào nên truy vấn có thể tóm nhầm phần tử của câu đang đi ra. |
> | Bàn thử | `scratch/dot302-fight-voice.html` (2 game × 2 chế độ) · `scratch/dot302-sweep.html` (quét 7 game, đổi template bằng chính `convert.js`). **A/B thật** (stash bản vá rồi chạy lại): **4/27 TRƯỢT trước → 27/27 ĐẠT sau**, 4 phép trượt rơi đúng 2 triệu chứng × 2 game. Chế độ **một bàn** ĐẠT sạch ở **cả hai** lượt ⇒ nếp ngoài trận không xê dịch một ly. `node --input-type=module --check` sạch cả 2 file. |
> | ⚠️⚠️ BÀN THỬ NÓI DỐI 4 LẦN | Đọc kỹ trước khi viết bàn thử tiếng: (1) nút loa nằm ở `.aw-fight-board`, **không phải** `.aw-fight-half`; (2) đếm mọi `<audio>` là đếm cả tiếng động của `core/sfx.js` — phải lọc theo data: URL của clip; (3) ⛔⛔ ép **`muted = true`** để qua cửa autoplay thì Chrome coi media đang tắt tiếng ở **tab nền** là *"video-only background media"* và **tự tạm dừng**, `play()` trả `AbortError`, `createVoicePlayer.start()` rơi vào `.catch` và **tắt hào quang** ⇒ bảng đo ra "không nút nào sáng" ở cả hai game, một kết luận của bàn thử hỏng. Dùng **`volume = 0`** + **một cú bấm THẬT** của người; (4) bắn cả `pointerdown` lẫn `click` thì `core/press.js` ăn **cả hai** ⇒ mỗi "cú chạm" thành hai lượt toggle. ⭐ Và phải có **mỏ neo DƯƠNG** (khẳng định nút bàn trái ĐANG sáng), không thì phép so "hai nút cùng trạng thái" ĐẠT một cách RỖNG. |
> | Quét 7 game | `quiz` ✅ (sau vá) · `find_the_match` ✅ (vốn đã đúng) · **5 game KHÔNG ĐO ĐƯỢC** — `anagram` · `true_false` bản chuyển đổi · `type_the_answer` · `open_the_box` · `crossword`: nút loa của chúng không có mặt ngay lúc mount (Crossword phải chạm ô số, Open the box phải mở hộp…). Khai đúng "không đo được", **không khai ĐẠT**. |
> | ⬜ Chờ tay thầy | (a) Một trận Fight act VOICE ở **Quiz** rồi **True/false**: hai nút loa phải sáng/tắt **cùng lúc**, chạm bên nào cũng chỉ **một** giọng đọc. (b) **Cho biết hôm nay thầy chơi game nào** — nếu là Anagram · Type the answer · Open the box · Crossword thì phải đo tiếp, 4 game đó đủ 4 mảnh trên giấy nhưng **chưa đo được hành vi thật**. |
> | Không đụng | 5 template còn lại · `core/fight.js` · `core/voice-playback.js` · engine · kho. Không thêm module mới. |
> | Chi tiết | `GHI CHU DU AN.md` Đợt 302. |
>
> ---
>
> ### 🕘 TRẠNG THÁI CŨ HƠN (07/9/2026 — sau **Đợt 301**)
>
> | | |
> |---|---|
...
> | | |
> |---|---|
> | Commit mới nhất | **`d3c6c8a`** (Đợt 291 — nhãn hiển thị quyết định MỘT LẦN cho cả trận, mọi độ dài tên) — `core/showdown.js` · `core/showdown-setup.js` · `core/engine.js` — ĐÃ PUSH + LIVE kiểm chứng (SHA-256 khớp `git show d3c6c8a` cả 3 file). |
> | Vì sao | Thầy: "không chỉ cùng 1 bảng, tôi cần mọi bảng khác nhau đều cần khác nhau, với mọi kiểu tên" — Đợt 290 chỉ né được ca 2 tiếng; tên 3+ tiếng vẫn có thể trùng giữa HAI ĐỘI KHÁC NHAU (VD "Nguyễn Bảo Anh"/"Ngô Bảo Anh"). Thầy tự chỉ hướng sửa: quyết định nhãn ngay lúc còn thấy CẢ TRẬN (lúc bấm Ready), đừng cố sửa ở màn READY nơi mỗi cột đã bị tách rời. |
> | Đã sửa | `assignFullLabels()` mới (`core/showdown.js`) — nhãn cho TOÀN BỘ tên trong một trận (mọi đội gộp lại) tính MỘT LẦN. `fullLabelMap()` (`core/showdown-setup.js`) gắn `pick.members[].shortLabel` ở cả 3 nơi pick lắp ráp: `stampPickFromTable` (sau publishTable, dùng bảng SERVER — bản CHUẨN, mọi cột cùng nguồn), `applyReady` (bản dự phòng trước publish), `applySolo` (1 đội = cả lớp). `sdRefreshFromTable()` (`core/engine.js`) cũng tính lại khi bảng đội đổi giữa chừng. Màn READY ưu tiên đọc `shortLabel` có sẵn, lùi về `shortenTeamNames()` (Đợt 290) cho pick cũ chưa có trường này. |
> | Kiểm | `node --check` sạch cả 3 file. Test độc lập: nguyên trận 2 đội giống ảnh chụp thầy gửi (Tuệ Lâm/Tùng Lâm) → hết trùng; tên 3 tiếng trùng GIỮA HAI ĐỘI KHÁC NHAU → tự mở đầy đủ, hết trùng; trùng tên thật 100% → đánh số (2). SHA-256 live khớp tuyệt đối cả 3 file. |
> | ⬜ Chờ tay thầy | Tạo 2 em tên 3 tiếng trùng chữ cái đầu (Settings › Classes), xếp vào HAI ĐỘI KHÁC NHAU, bấm Ready, mở 2 cột myActivity cạnh nhau tới màn READY — xác nhận hết trùng ở mọi độ dài tên. |
> | Chưa đụng | Bảng kết quả sau game (`fitPodiumNames`) vẫn như Đợt 289/290 note. |
> | Chi tiết | `GHI CHU DU AN.md` Đợt 291 (nối tiếp 289+290). |
>
> ---
>
> ### 🕘 TRẠNG THÁI CŨ HƠN (03/9/2026 — sau **Đợt 290**)
>
> | | |
> |---|---|
> | Commit mới nhất | **`0d4bb09`** (Đợt 290 — man READY vẫn trùng "T.LÂM" giữa 2 cột sau Đợt 289) — `core/engine.js` — ĐÃ PUSH + LIVE kiểm chứng (SHA-256 khớp `git show 0d4bb09`). |
> | Đã sửa | Tên 2 tiếng không còn viết tắt xuống 1 chữ cái theo mặc định — luôn hiện đủ ở mọi cột. ⬜ còn hở tên 3+ tiếng trùng giữa 2 đội — vá ở Đợt 291 ngay trên. |
> | Chi tiết | `GHI CHU DU AN.md` Đợt 290. |
>
> ---
>
...
> | | |
> |---|---|
> | Commit mới nhất | **`39b273b`** (Đợt 286b — sfx bộ tiếng → blob) ← `3faba1d` (Đợt 286: play.html đọc bài giao sớm + `core/tpl-files.js` mới + registry fetch bộ file template) — `core/assignments.js` · `core/registry.js` · `core/sfx.js` · `play.html` · `tools/sinh-preload.py` — **đã push + LIVE kiểm chứng** (SHA-256 5 file khớp HEAD). |
> | ⭐ Bàn giao cuối phiên 03/9 | Thầy đã duyệt cả Đợt 285 + 286, "việc nhỏ còn lại xử lý sau". Kho SẠCH, `main` = `origin/main` (`d91a42f`). Phía myLesson web đi kèm: **v1.54.2** (`04bcd07`) — khối SOM xin dữ liệu sớm + `bai.html` `suoiAWord()` kéo sẵn `play.html` + mọi modulepreload của AWord khi vẽ thẻ (đọc `myLesson/app/BAN GIAO.md` mục `0⚡`). ⬜ Còn chờ MẮT THẦY trên máy trường/TOMKO (mở sau >10 phút, bấm READY). ⬜ Ý tưởng chưa làm: myLesson tạo iframe ngầm ngay khi vẽ thẻ (nhanh nhất, nhưng tốn lượt đọc Firestore cho mỗi thẻ mỗi lần mở trang) — chờ thầy chốt. |
> | Kiểm live 286 | `play.html?g=3mj3v3&nhung=1` trên Chrome thật của thầy: đọc bài giao bắt đầu **57ms** (trước: sau khi mã chạy ~416ms); 4 file template về **một đợt** 280→287ms (trước: 2 vòng 566→1375ms); bộ tiếng 10 file **fetch song song 290→293ms, 0 lượt <audio> ra mạng**, `pack.stats()` 10/10, `src` là `blob:`. Ngay sau deploy (cả 39 module phải tải lại 351 KB): mã xong **448ms** — song song thật. ⚠️ Chrome trì hoãn media ở tab ẨN: đo `stats()` ở tab nền thấy `ready 0` vài giây đầu là bình thường, không phải lỗi. | ⛔ Thêm template/đổi import trong templates/ ⇒ `python tools/sinh-preload.py --write` rồi commit cả `core/tpl-files.js` + 2 HTML. |
...
> | | |
> |---|---|
> | Commit mới nhất | **`1f7eb8e`** (Đợt 280+281 — Showdown READY bố cục mới + Fight mode thêm thanh MISS WAIT) — **đã push + LIVE kiểm chứng** |
> | Kiểm live | Fetch trực tiếp cả 3 file đổi — **mã băm SHA-256 khớp tuyệt đối** với `git show 1f7eb8e:<file>` cho cả ba: `core/app.css` (`0e5c070c…529f22`), `core/engine.js` (`90436785…c5e20a`), `core/fight.js` (`d84f1e3a…23ad6c`). GitHub Pages deployment `6142206278` cho đúng SHA `1f7eb8e` báo `success`. |
> | Trước đó | **`f768429`** (Đợt 279 — Showdown: đổi template ở màn setup nay rút khỏi `sd_round` cũ) — đã push + LIVE kiểm chứng |
> | Kho | **SẠCH**, `main` khớp `origin/main` (ahead 0 · behind 0) |
>
> ### ⭐⭐ ĐỢT 280 — VÙNG VỪA ĐỘNG TỚI: **SHOWDOWN READY — BỐ CỤC MỚI (CỘT TÊN TRÁI, PLAY TO GIỮA)**
>
> Thầy chốt qua xem thử **Artifact** (mockup HTML so sánh HIỆN TẠI/ĐỀ XUẤT, 3 vòng chỉnh theo phản
> hồi), không phải qua yêu cầu bằng lời một lần: *"chỉnh một chút về giao diện màn hiện START và hiện
> tên học sinh"* → sau 3 vòng góp ý (chữ to hơn + viết tắt, PLAY to giữa/template trên/số câu dưới,
> tên lesson 1 dòng ở đáy) → *"Ok chốt cái này"*.
>
> **File cốt lõi: `core/engine.js`** (khối lắp ráp `if (showdownPick) {...}` ngay trước
> `playOverlay.append(readyCenter)`) **+ `core/app.css`** (`.aw-sd-namecol`/`.aw-sd-contentcol`/
> `.aw-sd-topstrip`/`.aw-sd-bottomstrip`). Chỉ đổi bố cục khi `showdownPick` — mọi chế độ khác (chơi
> thường, homework, "Start with mistakes") giữ NGUYÊN màn READY cũ, không đụng một dòng.
>
> Bốn thay đổi, đúng thứ tự thầy chốt:
> 1. **Cột tên học sinh 1/3 trái** — xếp dọc, viết hoa, chữ to (`calc(4.2 * var(--aw-u))`), có vạch
>    chia mảnh (`.aw-sd-namecol::after`) tách khỏi 2/3 phải.
> 2. **Tên viết tắt "tên gọi"** — hàm mới `shortenTeamNames()` (cuối `core/engine.js`): giữ 1-2 tiếng
>    cuối tên đầy đủ; nếu 2 em CÙNG ĐỘI trùng tên gọi 1 tiếng thì tự mở rộng thành 2 tiếng để phân
>    biệt (không cần thầy tự đặt tên hiển thị riêng).
> 3. **2/3 phải: template → PLAY (to, `calc(20 * var(--aw-u))`, giữ NGUYÊN phong cách `.aw-bigplay`
>    gốc — chỉ phóng cỡ, không đổi màu/hiệu ứng) → số câu**, đúng thứ tự thầy chốt.
> 4. **Ô tích "đội nào sẵn sàng" (Đợt 263) dời lên dải trên cùng** (`.aw-sd-topstrip`, thu nhỏ) — tên
>    lesson (Đợt 154, từng là dòng to nhất đứng đầu đội hình) dời xuống **1 dòng ở dải đáy, thấp
>    nhất** (`.aw-sd-bottomstrip`, cắt bằng "…" nếu quá dài).
>
> ⚠️ Cả 2 dải mới (`aw-sd-topstrip`/`aw-sd-bottomstrip`) giữ ĐÚNG quan hệ cha-con cũ với phòng chờ
> (Đợt 261/263): dải trên là CON TRỰC TIẾP của `playOverlay` (ngang hàng `readyCenter`) nên vẫn sống
> suốt phòng chờ; dải đáy là CON của `readyCenter` nên ẩn/hiện cùng đội hình lúc vào/ra phòng chờ —
> không sửa một dòng nào trong `enterLobby()`/`cancelLobby()`.
>
> Kiểm bằng trang test tĩnh `scratch/dot280-sd-ready-check.html` (không commit — dựng DOM giống hệt
> khối lắp ráp thật, dùng thẳng `core/app.css` thật qua `<link>`, KHÔNG viết lại CSS): chụp màn khớp
> đúng bản mockup thầy đã duyệt (cột tên trái + vạch chia + PLAY to giữa + ô tích trên + lesson dưới).
> `node --input-type=module --check` sạch trên `core/engine.js`.
>
> ✅ **ĐÃ PUSH + LIVE KIỂM CHỨNG** (`1f7eb8e`). ⬜ **CHỜ THẦY BẤM TAY THẬT** — bàn thử chỉ soi CSS/DOM
> tĩnh, chưa ai mở một trận Showdown 4 cột thật trên TOMKO.
>
> ---
>
> ### ⭐⭐ ĐỢT 281 — VÙNG VỪA ĐỘNG TỚI: **FIGHT MODE — THANH MISS WAIT (GRADIENT, DẢI TRÊN)**
>
> [[Đợt 276]] đã dựng ĐỒNG HỒ của MISS WAIT (thanh trượt Options 0–20s + ∞, thay `LATE_LIMIT_MS` cứng)
> nhưng KHÔNG có gì vẽ lên màn hình. Thầy: *"đội bên trái làm trước bị sai, thì đội sau ko thể đợi
> mãi tùy ý được ... cần chạy 1 thanh thời gian có đổi màu gradient ở dải phía trên để thể hiện lượng
> thời gian còn lại"*. Xác nhận qua hỏi lại 2 lựa chọn trước khi code: thanh chỉ hiện ở **bàn còn
> được chơi tiếp** (không phải cả hai bàn), và màu theo **3 mức xanh→cam→đỏ giống thanh Showdown có
> sẵn** (không phải màu chuyển mượt liên tục).
>
> **File cốt lõi: `core/fight.js`** (đồng hồ + pixel — KHÔNG qua `core/engine.js` như thanh chờ
> tie-window cũ, vì thanh này sống Ở DẢI TRÊN, đất `fight.js` tự dựng sẵn — `top`/`half0`/`half1`,
> cùng chỗ với điểm số mỗi đội và thanh PICK TIME của Đợt 259, không phải hàng nút dưới của
> `engine.js`) **+ `core/app.css`** (`.aw-fight-missbar`/`.aw-fight-missbar-fill`, nằm mép TRÊN của
> `.aw-fight-half` — đối diện mép dưới của pickbar nên không bao giờ đè lên nhau).
>
> Kỹ thuật: `missBars` — hai phần tử (một mỗi bàn), CÙNG KHUÔN hai-node với `pickBars` (Đợt 259) và
> `.aw-waitbar` gốc (Đợt 187) — bề rộng là MỘT CSS transition duy nhất theo đúng độ dài `wrongWaitMs`,
> không có vòng lặp mỗi khung hình. Dải màu xanh→cam→đỏ (cùng ngưỡng % với `.aw-roundbar-fill`) do
> **hai `setTimeout()`** canh đúng mốc 50%/20% còn lại — không phải một tick loop — và được **đặt lại
> từ ĐIỂM HIỆN TẠI** (không phải từ đầu) mỗi khi bật thanh hay "go" sau một lượt tạm dừng, nên một lần
> mở ☰ Menu giữa chừng không làm màu nhảy lùi về xanh. `paintMissBar(side, ms, mode)` — ba trạng thái
> "start"/"hold"/"go", `hold`/`go` gọi từ `setRefPaused()` cùng nhịp với `roundDue` (đồng hồ THẬT của
> MISS WAIT, không cần một `Due` timer riêng — chỉ thêm biến `missWaitSide` để nhớ bàn nào đang hiện
> thanh). Thanh tự tắt NGAY lúc bàn được đếm trả lời xong (hook đầu `wordDone`), không đợi tới
> `advanceRound()` — vốn có thể còn cách `ROUND_HOLD_MS` (~2,1s) nữa mới chạy.
>
> ⚠️ Dưới ngưỡng `WAIT_BAR_MIN_MS` (200ms) — gồm cả mốc 0s hợp lệ của Miss wait (cắt tức thì) — KHÔNG
> vẽ thanh, chỉ tổ nháy một khung hình. Ở ∞ trọng tài không hề gọi `later()` (né đúng bẫy
> `setTimeout(fn, Infinity)` nổ NGAY TICK SAU) nên thanh cũng không bao giờ bật.
>
> Kiểm bằng trang test tĩnh `scratch/dot281-fight-missbar-check.html` (không commit — dựng
> `half0`/`half1` giống hệt `core/fight.js` thật, dùng thẳng `core/app.css` qua `<link>`, bản rút gọn
> trực tiếp từ `paintMissBar`): đo timing THẬT qua `javascript_tool` (không đoán) trên cửa sổ 5000ms —
> `is-orange` bật đúng lúc **2804ms** (mốc lý thuyết 2500ms = 50%), `is-red` bật đúng lúc **4107ms**
> (mốc lý thuyết 4000ms = 20% còn lại), cạn về 0px lúc 5321ms, và tắt mờ dần đúng transition
> `opacity .16s` (đọc `getComputedStyle` sau 250ms → `opacity:0`). `node --input-type=module --check`
> sạch trên `core/fight.js`.
>
> ⚠️ **SỐ THỨ TỰ ĐỢT**: bản nháp ban đầu đặt tính năng này là "Đợt 277" (đếm tiếp từ Đợt 266, số cuối
> cùng phiên này biết tới trước khi đọc lại repo) — trùng với Đợt 277 (âm trả lời sai) đã có sẵn trên
> `origin/main` từ một phiên khác. Phát hiện qua `git fetch` + so `git log --all` TRƯỚC KHI COMMIT
> (đúng luật bắt buộc), đổi toàn bộ comment trong `core/fight.js`/`core/app.css` sang 281 bằng `sed`
> có kiểm lại trước khi commit — không có dòng "Đợt 277" nào của tính năng này lọt vào lịch sử.
>
> ✅ **ĐÃ PUSH + LIVE KIỂM CHỨNG** (`1f7eb8e`). ⬜ **CHỜ THẦY BẤM TAY THẬT** — bàn thử chỉ đo trên
> harness tĩnh, chưa ai chơi một trận Fight thật, cho một đội trả lời sai và nhìn thanh chạy trên máy
> thật (đặc biệt TOMKO — màu xanh/cam/đỏ phải phân biệt được từ cuối lớp).
>
> ---
>
> ### 🕘 TRẠNG THÁI CŨ HƠN — GIỮ LÀM LỊCH SỬ (28/8/2026 — sau **Đợt 279**)
>
> | | |
> |---|---|
...
> ⭐ **(Đợt 404, 26/9/2026) — A SHOW SPEED Fight Mode 1: bàn thua hiện từ đúng của đội kia.** ⬜ thầy bấm tay TOMKO (độ rõ chữ trắng trên bàn xám, nhịp giữ 2,1 s).
>
...
> ---
>
> ⭐⭐⭐⭐⭐⭐ **MỚI NHẤT (Đợt 368, 22/9/2026) — ROCKET RACE ▸ FIGHT TRÊN HAI MÁY (câu hỏi sang iPad). CHƯA COMMIT.**
> ⚠️ **ĐƯỜNG MẠNG THẬT CHƯA CHẠY LẦN NÀO.** Claude kiểm được toàn bộ phần nhìn + phép thử an toàn, nhưng KHÔNG
> đăng nhập được tài khoản Google của thầy nên hai máy chưa bao giờ nối thật. Đừng coi là xong.
> A. ⬜ **DUYỆT ĐỀ XUẤT SỬA CORE**: `core/store.js` thêm 3 chuỗi vào `APP_DATA_KINDS` (`rocketrace-link`,
>    `rocketrace-view`, và vá `showdown-session` thiếu từ Đợt 269). Không đổi logic. Chính file đó yêu cầu.
> B. ⬜ **7 bước bấm tay** (chi tiết ở `templates/rocket-race/GHI CHU ROCKET-RACE.md` mục 20): Options trận tích
>    **Question screen** → iPad mở `…/source.html` đăng nhập → PLAY ⇒ trong ~20 s câu hỏi rời máy chơi sang iPad →
>    đổi câu vài lượt → Fight content = **Different** phải ra 2 nửa khác nhau → **tắt wifi iPad 1 phút** ⇒ iPad báo
>    `⚠ OFFLINE` và máy chơi **tự hiện lại** câu hỏi → hết trận iPad về "Waiting for the game".
> C. ⬜ Safari trên iPad: nút **Sign in** dùng popup Google — nếu iPad chặn popup thì phải đổi sang `signInWithRedirect`.
> D. ⬜ TOMKO thật (chưa test lần nào, cùng nợ với Đợt 353-356).
> E. ⬜ Thầy xem có muốn iPad hiện thêm gì không (nay: chữ câu hỏi + tên/màu đội + "n / N" + đồng hồ trận).
>    ⛔ Đã CỐ Ý không đưa thanh Time delay / Miss wait lên iPad — xem lý do ở mục 20.
> F. ⬜ Sau khi chạy thật ổn: có mở cho các template Fight khác không (nay chỉ Rocket race — thầy chốt vậy).
>
> ---
>
> ⭐⭐⭐⭐⭐ **(Đợt 353 + 354, 20/9/2026 tối) — ROCKET RACE ▸ FIGHT TỈ LỆ MỚI 32:7 + 16:7, KHÔNG ĐIỂM, SỐ MŨI TÀU, LIVES = NÁT → NỔ: ✅ COMMIT + PUSH `f5fd42e`, ⬜ CHƯA BẤM TAY TRANG THẬT.**
...
> 3. ⬜ Nhìn tên lửa: phi công 🐱/🦊 nằm TRONG cửa sổ; lửa đi theo thân khi nhấp nhô / lao tới (đúng) / rung (sai). Kiểm cả Solo.
...
> ⭐⭐⭐⭐⭐ **(Đợt 351, 20/9/2026 chiều) — ROCKET RACE ▸ FIGHT: ✅ COMMIT `cb95795`, ⬜ CHƯA BẤM TAY TRANG THẬT.**
...
>    còn tiếng lệch nhau.
> 2. ⬜ Lặp lại y vậy với **True/false**.
> 3. ⬜⭐ **Thầy cho biết hôm nay chơi game nào.** Nếu là Quiz hoặc True/false thì đợt này đúng
>    bệnh. Nếu là **Anagram · Type the answer · Open the box · Crossword** thì phải đo tiếp:
>    4 game đó có đủ 4 mảnh **trên giấy** nhưng bàn thử Đợt 302 **chưa đo được hành vi thật**
>    của chúng (nút loa của chúng không có mặt ngay lúc mount).
> 4. ⬜ Test chạm **TOMKO**.
>
...
> ---
>
> ⭐⭐⭐ **Đợt 301 (07/9/2026) — code `3d2a7cd`, ĐÃ PUSH + LIVE kiểm chứng
...
> (xanh→cam→đỏ, dải trên, chỉ bàn còn chơi tiếp). Bản đồ đầy đủ ở mục **0a ▸ ⭐⭐ ĐỢT 280**
> và **▸ ⭐⭐ ĐỢT 281**.
>
> **Chờ TAY/MẮT thầy — cả hai đều mới chỉ soi qua bàn thử tĩnh (DOM/CSS + timing đo bằng
> `javascript_tool`), chưa ai chạm một trận thật:**
> 1. ⬜ **(280)** Mở một trận Showdown thật (4 cột hoặc trên TOMKO) — cột tên học sinh bên
>    trái phải to, viết tắt đúng, đọc được từ xa; PLAY to giữa đúng bố cục đã duyệt qua
>    Artifact; tên lesson gọn 1 dòng ở đáy.
> 2. ⬜ **(281)** Mở một act Fight bất kỳ, cho một đội trả lời SAI trước, đội kia CỐ TÌNH
>    không bấm — thanh MISS WAIT phải chạy trên đúng bàn còn chơi tiếp (không phải bàn vừa
>    sai), đổi màu xanh→cam→đỏ đúng lúc, và biến mất ngay khi hết giờ hoặc khi đội kia bấm.
> 3. ⬜ **(281)** Mở ☰ Menu giữa lúc thanh đang chạy — thanh phải ĐỨNG YÊN (không chạy tiếp
>    ngầm), và chạy tiếp đúng phần còn lại khi đóng Menu.
> 4. ⬜ Test chạm **TOMKO** cho cả hai — đặc biệt màu xanh/cam/đỏ của (281) phải phân biệt
>    được từ cuối lớp.
>
> ---
>
> ⭐⭐⭐ Đợt 279 (28/8/2026) — code `f768429`, ĐÃ PUSH + LIVE kiểm chứng, chờ tay thầy thử
> **SHOWDOWN — đổi template ở màn setup** (dựng 4 cột, để mặc định Anagram và bấm START một
> lần, thoát ra màn setup rồi đổi cả 4 sang Quiz, bấm Play lại phải vào ĐÚNG Quiz, lặp lại
> vài lần không tái phát; test TOMKO) — chi tiết ở khối Đợt 279 trong `GHI CHU DU AN.md`.
>
> ---
>
