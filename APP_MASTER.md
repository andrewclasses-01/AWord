...
> Cập nhật lần cuối: **23/9/2026 (Đợt 377 — KHÔNG ZOOM: `touch-action: manipulation` cho mọi phần tử trong `app.css` + `core/no-zoom.js` chặn 2 ngón/Ctrl+lăn/Ctrl ± 0 + viewport `user-scalable=no`)**. Trước đó Đợt 376 — PWA (manifest chỉ ở index.html, icon iPhone + maskable).
