...
> ⭐ **26/9/2026 Đợt 406 — ROCKET RACE FIGHT 3D: bỏ chữ LIFTOFF · Apply Options KHÔNG chạy lại cảnh phóng (cảnh đua + START 3D hiện sau ~0,4 s thay vì ~2,5 s dựng + ~10 s xem) · sau START chạm ĐÚP bất kỳ đâu ⇒ tua intro tới ~3 s trước câu hỏi** (chỉ `templates/rocket-race/`; ⬜ CHƯA PUSH; ⬜ chưa bấm tay TOMKO). Chi tiết chặng Đợt 406.
> ⭐ **26/9/2026 Đợt 405 — ROCKET RACE act VOICE: chạm thanh câu hỏi 3D (chữ 🔊) = nghe lại voice (Different: nửa trái/phải); act voice ⇒ nhạc nền TẮT cứng, dòng BACKGROUND trong menu 🔊 khoá mờ (không ghi đè lựa chọn đã nhớ), 2D cũng không bật nhạc** (chỉ `templates/rocket-race/`; ⬜ CHƯA PUSH; ⬜ chưa bấm tay TOMKO). Chi tiết chặng Đợt 405.
