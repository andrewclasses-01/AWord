...
1. Mở **https://console.firebase.google.com** → đăng nhập **bằng Google account thầy sẽ dùng để
   quản lý AWord** (nhớ kỹ dùng account nào, bước 4 sẽ cần).
...
3. ⚠️ **Sửa 1 chỗ**: thay `THAY-EMAIL-CUA-THAY@gmail.com` bằng đúng email Google thầy dùng ở BƯỚC 1.
4. Bấm **Publish**.
  ...
  match /databases/{database}/documents {
    ...
    function isTeacher() {
      return request.auth != null
          && request.auth.token.email == 'THAY-EMAIL-CUA-THAY@gmail.com'
...
1. Đoạn `firebaseConfig` ở **BƯỚC 5**.
2. Xác nhận **email Google** thầy đã điền vào luật ở **BƯỚC 6**.
